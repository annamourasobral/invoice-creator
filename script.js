...
function sum() {
    ...
    totalAmount.innerHTML = `
        <td class="note">We accept cash, credit card, or PayPal</td>
...
function addService(option) {
    if( !servicesArr.includes(option.service) ) {
        purchasedServices.innerHTML += `
            <tr>
                <td class="service">${option.service} <span class="remove" onclick="remove('${option.service}')">&emsp;Remove</span></td>
                <td align="right" class="price"><span class="dolar-sign">$</span>${option.price}</td>
            </tr>
        `
